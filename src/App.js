...
class App extends React.Component {
  ...
  handleClick(i) {
    const questionHistory = this.state.questionHistory.slice(
      0,
      this.state.stepNumber + 1
    );
    const currentQuestion = this.getByNodeId(
      questionHistory[this.state.stepNumber].nodeId
    );
  ...
  renderQuestion(props) {
    ...
    let currentQuestion = this.getByNodeId(
      questionHistory[this.state.stepNumber].nodeId
    );
    return (
      <Question
  ...
  getTreeImagePath(treeName) {
    return (
      process.env.PUBLIC_URL + `./img/${treeName}/full.jpg`.replace(" ", "-")
    );
  }
  ...
  render() {
    ...
    const floatingButton = (
      <FloatingResetButton handleClick={() => this.jumpTo(0)} />
    );
    return (
